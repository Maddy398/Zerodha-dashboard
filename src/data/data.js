...
// ----------------------------------
// Function to simulate market updates
// ----------------------------------

function getRandomChange(value, percentRange = 3) {
  const changePercent = (Math.random() * percentRange * 2 - percentRange) / 100;
  return +(value + value * changePercent).toFixed(2);
}

export function updateMarketData() {
  watchlist = watchlist.map((item) => {
    const newPrice = getRandomChange(item.price);
    const priceChange = newPrice - item.price;
    const percent = ((priceChange / item.price) * 100).toFixed(2) + "%";
    const isDown = priceChange < 0;
    return { ...item, price: newPrice, percent, isDown };
  });

  holdings = holdings.map((item) => {
    const newPrice = getRandomChange(item.price);
    const net = (((newPrice - item.avg) / item.avg) * 100).toFixed(2) + "%";
    const day = (((newPrice - item.price) / item.price) * 100).toFixed(2) + "%";
    const isLoss = day.startsWith("-");
    return { ...item, price: newPrice, net, day, isLoss };
  });
